...
lowess.fragment.length <- function(jtkx, jtky) {
        jtklow <- lowess(jtkx, log(jtky), f=0.05)
        jtkz <- approx(jtklow$x, jtklow$y, jtkx)
        return(exp(log(jtky) - jtkz$y))
}
...
###################
	hg_dm_ratios <- integer(ncol(thisUber))
	dm_start <- min(which(grepl("dm", gc[,1]), TRUE))
	hg_autosome_end <- min(which(grepl("chrX", gc[0:dm_start,1]), TRUE))
	dm_autosome_end <- min(which(grepl("chrX_dm", gc[,1]), TRUE))
	print(hg_autosome_end)
	print(dm_start)
	print(dm_autosome_end)
########################
	...
	for (i in 1:ncol(thisUber)) {
		...
		thisLowratio <- lowess.fragment.length(lengths$median.len.ratio, thisLowratio_1)
		
		thisUberRatio[, i] <- thisLowratio
###################
		#get the mean counts for hg and dm separately and find ratio
		hg_mean <- mean(thisLowratio[0:hg_autosome_end])
		dm_mean <- mean(thisLowratio[dm_start:length(thisLowratio)])
		hg_dm_ratios[i] <- dm_mean / hg_mean
########################		
		...
		thisUberSeg[, i] <- m[, 1]
		write.table(thisShort, sep="\t", file=paste(outdir, "/", sample.name, ".hg19dm6.len.norm.short.txt", sep=""), quote=F, row.names=F) 

		chr <- chrom.numeric
		chr.shift <- c(chr[-1], chr[length(chr)])
		vlines <- c(1, abspos.col[which(chr != chr.shift) + 1], abspos.col[nrow(thisUber)])
		hlines <- c(0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0)
		...
		points(x=abspos.col, y=thisUberSeg[, i], col="#0060a9")
		lines(x=abspos.col, y=thisUberSeg[, i], col="#0060a9")
		abline(h=hlines)
		abline(v=vlines)
		...
		thisLowratioQuantal <- thisLowratio * 2 #thisMultiplier
		thisSegQuantal <- thisUberSeg[, i] * 2 #thisMultiplier

		thisUberRatioQuantal[, i] <- thisLowratioQuantal
		thisUberSegQuantal[, i] <- thisSegQuantal
	
		hlines <- c(0, 0.5, 1, 1.25, 1.5, 1.75, 2, 2.5, 3, 3.5, 4, 4.5, 5, 6)
		...
		write.table(thisRatioOut, sep="\t", file=paste(outdir, "/", sample.name, ".hg19dm6.len.norm.varbin.data.txt", sep=""), quote=F, row.names=F)

	}
##############
	today <- Sys.Date()
	form <- format(today, format="%d")
	df_ratios <- data.frame(t(hg_dm_ratios))
	colnames(df_ratios) <- colnames(thisUber)
	write.table(df_ratios, sep="\t", file=paste(outdir, "/", "uber.hg19dm6.count.ratios_125_600.txt", ".txt", sep=""), quote=F, row.names=F)
